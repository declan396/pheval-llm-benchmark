"""
run_llm__results_rag_exomiser.py
=======================
...
MAX_TOKENS        = 1000
RESULTS_DIR       = Path("llm_results_rag_exomiser")
...
def _pad_genes(top_genes: list, retrieved: list, exomiser_genes: list, target: int = 10) -> list:
    """Pad to target using Exomiser genes then RAG genes."""
    seen = {g["gene_symbol"] for g in top_genes}

    # First pad from Exomiser
    for g in exomiser_genes:
        if g["gene"] not in seen and len(top_genes) < target:
            top_genes.append({
                "rank":        len(top_genes) + 1,
                "gene_symbol": g["gene"],
                "score":       round(g["phenotype_score"] * 0.5, 4),
                "reasoning":   f"Exomiser phenotype score: {g['phenotype_score']}",
            })
            seen.add(g["gene"])
        ...
        for g in d["genes"]:
            if g and g not in seen and len(top_genes) < target:
                top_genes.append({
                    "rank":        len(top_genes) + 1,
                    "gene_symbol": g,
                    "score":       round(d["similarity"] * 0.3, 4),
